    ...
    for publisher,URL in pages.items():
        ...
        for url in [URL+s for s in suffix]:
            ...
            for i,l in enumerate(links):
                ...
                if not published:
                    print('Saving in db...')
                    session.add(Article(Author=None, Claps=None, Reading_time=None, Title=None, Text=None, Labels=None, Publication = publisher,\
                                        Time = datetime.strptime(times[i], "%Y-%m-%dT%H:%M:%S.%fZ"), Link = l, Date_Scrapping=date.today(),\
                                        Images=None, Links=None, Code_Chunks=None, Numbered_Lists=None, Bullet_Lists=None, Bolded=None, Italics=None))
                    session.commit()
